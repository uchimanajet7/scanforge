...
- 最終更新: 2025-12-20
- 状態: 開発中（破壊的変更の可能性あり）
- 位置づけ（SSOT）: 本書は **現行実装の仕様** を記述する Single Source of Truth
...
## 1. 概要
ScanForge は、バーコード/QR の **読み取り** と **生成** を提供する「静的 WebUI（`web/`） + AWS Lambda API（`lambda/`）」です。
...
- テーマ: ライト固定（ダークテーマは非対応 / OS 設定に追従しない）
- OGP: `web/index.html` のメタで `web/ogp.png` を参照する
...
### 3.1 共通
- 操作結果（成功/失敗）は画面上のフィードバック（メッセージ/トースト等）で明確に提示する。
...
- ベース URL:
  ...
  - ローカル: `http://127.0.0.1:8001`（`bash scripts/tools/api/start-local-api.sh`）

レスポンスの基本形
- 成功: HTTP 200 + JSON
- 失敗: HTTP 4xx/5xx + JSON
...
- `barcodeFormat`（任意, string）: `svg|png|jpeg|webp`
  - 指定がない場合は Content-Type / 先頭タグ等から判定する

#### 制約（`barcodeUrl`）
- `https` のみ許可（`http`/`file` 等は拒否）
- リダイレクトは最大 3 回まで
