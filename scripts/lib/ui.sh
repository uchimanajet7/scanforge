#!/usr/bin/env bash
# 共通 UI ライブラリ。ScanForge のスクリプト群で統一するログ出力は `[tag] message` 形式。
set -euo pipefail
...
ui::section_start() { # 引数: $1=格納先変数名 $2=タグ $3=ログラベル。省略できます。
  local __var="$1"; shift
  local tag="$1"; shift
  local label="${1:-start}"
  [[ -n "$label" ]] || label="start"
...
ui::section_end() { # 引数: $1=タグ $2=開始時刻。単位はミリ秒です。 $3=ログラベル。省略できます。
  local tag="$1"; shift
