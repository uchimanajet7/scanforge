...
# ============================================================================
# ローカル API 起動ラッパー
# - 通常: venv セットアップ → ローカル API サーバー起動（フォアグラウンド）
# - --auto: 起動→ready確認→スモーク→停止（完全自動）
# ============================================================================
...
説明:
  --auto は「起動→ready確認→/encode→/decode スモーク→停止」までを 1 回で実行します（プロンプトなし）。
  既存プロセスが同一ポートを待受している場合は、確認なしで停止（SIGTERM→必要なら SIGKILL）してから続行します。
USAGE
}
...
stop_server() {
  ...
  ui::info "${LOG_TAG}" "ローカル API を停止します（PID=${RUN_WRAPPER_PID}）"
  ...
  while kill -0 "${RUN_WRAPPER_PID}" 2>/dev/null; do
    ((loops++))
    if (( loops > 20 )); then
      ui::warn "${LOG_TAG}" "停止が完了しないため強制終了を試みます（PID=${RUN_WRAPPER_PID}）"
...
wait_for_ready() {
  ...
  while true; do
    ...
    if [[ -n "${code}" && "${code}" != "000" ]]; then
      ui::ok "${LOG_TAG}" "ready: ${url}（status=${code}）"
      return 0
    fi

    if (( loops > 100 )); then
      ui::warn "${LOG_TAG}" "ready 確認がタイムアウトしました（10s）。続行します。"
      return 0
    fi
...
if [[ ${SMOKE_RC} -eq 0 ]]; then
  ...
  ui::err "${LOG_TAG}" "スモーク: NG（rc=${SMOKE_RC}）"
fi
